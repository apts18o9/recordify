...
//arject implementation
// export const {GET, POST} = toNextJsHandler(auth.handler)


//email validation for stopping from fake emails, so block any fake email
...
//to protect from unwanted ip's and other
...
const protectedAuth = async (req: NextRequest): Promise<ArcjetDecision> => {
    ...
    if(session?.user?.id){
        ...
        userId = ip(req) || '127.0.0.1' //default ip
    }
